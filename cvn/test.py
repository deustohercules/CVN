...
def test():
    ...
    for i in files:
        if i.endswith(".xml"):
            xml_files.append('examples/'+i)
            name_file.append(i)
    for name, file in enumerate(xml_files):
        ...
        with open("mappings/cvn/1.4.2_sp1/cvn-to-roh/entities.toml") as f:
            ...
            try:
                ...
                print(e)
                return "Error while parsing the .toml."
        ...
        string_destino = 'examples/result/' + name_file[name][:-4] + '.ttl'
        g.serialize(format=params['format'],destination=string_destino)
    return True
